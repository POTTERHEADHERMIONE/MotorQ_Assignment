...
// Define the sendNotification function outside of handleReserveButtonClick
const sendNotification = (message) => {
    fetch('http://localhost:3002/notify', {
        ...
        body: JSON.stringify({ message }), // Message is provided here
    })
...
const handleReserveButtonClick = async (event) => {
    if (!user.email) {
        ...
        try {
            // Update the car count in Firestore
            ...
            const emailParams = {
                ...
                car_name: carBrand,
                car_model: carModel,
                start_date: rentDate.start,
                end_date: rentDate.end
            };
            await emailjs.send('service_ldyz4v3', 'template_66ggs3a', emailParams, 'gPhpHSiKRPrrhasdW');
    
            // Send notification to the second application
            sendNotification(`New reservation: ${carBrand} ${carModel} reserved from ${rentDate.start} to ${rentDate.end}`);
                            ...
                            <Row>
                                <Col>
                                    <div style={{ display: 'flex', justifyContent: 'center' }}>
                                        <Button
                                            style={{ backgroundColor: 'rgba(74, 209, 224, 0.581)', width: '300px',marginTop:'20px' }} // Reduced width
                                            variant="success"
                                            size="lg"
                                            className="fs-4 fw-bold" // Removed `w-100` to ensure width is controlled by inline style
                                            type="button"
